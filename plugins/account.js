...
internals.applyRoutes = function(server,next) {
    ...
    /*
        Get list of accounts by username.
        Get all accounts if username is empty
        Support paging / sorting
    */
    server.route({
        method: 'GET',
        path: '/accounts',
    ...
    server.route({
        ...
        handler: function (request, reply) {
            Account.findById(request.params.id, (err, account) => {
                ...
                if (!account) {
                    return reply(Boom.notFound('Account document not found'))
                }
                reply(account);
            })
        }
    ...
    server.route({
        ...
        handler: function (request, reply) {
            ...
            Account.findById(id, fields, (err, account) => {
                ...
                if (!account) {
                    return reply(Boom.notFound('Account document not found. That is strange'))
                }
                reply(account);
            })
        }
    ...
    */
    ...
    /*
        Link an existing user to an account
    */
    server.route({
        method: 'PUT',
        ...
        config: {
            ...
            pre: [
                ...
                },
                {
                    assign: 'userCheck',
                    method: function(request, reply) {
                        if (request.pre.account.user && request.pre.account.user.id != request.pre.user._id.toString()) {
